...
void simulation(std::vector<double> temperatures, Spin h, Spin k, int Lx, int Ly, int Lz, float J, int64_t Nmax, int Ns, float Time){
    ...
    std::string filename_wolff = "Wolff_Ghost, Lattice Size " + to_string(Lx) + "x" + to_string(Ly) + "x" + to_string(Lz) + ", h_z " + to_string(h(2)) + ", k_z " + to_string(k(2)) + ".txt"; //Create .txt file with name "mag_and_energy_T.txt"


    std::ofstream outFile1(filename_wolff);
    if (outFile1.is_open()) {
        // Write output data to file
        outFile1 << filename_wolff << endl << "Wolff_Ghost" << endl << "L = " <<  Lx << "," << Ly << "," << Lz << endl << "h = " << h(0)<< "," << h(1) << "," << h(2) << endl
        << "k = " << k(0)<< "," << k(1) << "," << k(2) << endl << "Ns = " << Ns << endl << "Nmax = " << Nmax << endl << "No_of_datapoints = " << number_of_steps << endl;
        for (size_t i = 0; i < temperatures.size(); ++i) {
