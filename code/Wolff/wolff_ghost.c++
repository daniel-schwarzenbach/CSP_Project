...
//     flt a = (matrix * matrix.transpose() - Matrix3x3::Identity(3,3)).norm();
...
bool activate_ghost_spin(Spin& spin_x, Spin& spin_r, flt beta, Matrix3x3& ghost, Spin& H){
    ...
    flt active = std::max(0.0, 1 - std::exp(- beta *(B1 - B2) ) );
    flt p = rng::rand_uniform();
    return (p <= active);
}
...
bool activate_spin_ghost(Spin& spin_x, Spin& spin_r, flt beta, Matrix3x3& ghost, Spin& H){
    ...
    Vector3 mag_field = H;
    flt B1 = mag_field.dot(ghost_inverse * spin_x_r); //B(s) = H^T * s = H^T * (R)^-1 * (r*sigma_x)
    flt B2 = mag_field.dot(ghost_inverse * spin_x); //B(s) = H^T * s = H^T * (R)^-1 * (sigma_x)
...
int wolff_ghost_algorithm(Lattice& lattice, flt beta, Spin H){
    ...
    while(!stack.empty()){
        ...
        if(!visited[cx][cy][cz]){
            ...
            check_neighbor_ghost(lattice, (cx+1+Lx)%Lx, cy, cz, spin_x, spin_r, visited, stack, cluster, beta);
            check_neighbor_ghost(lattice, (cx-1+Lx)%Lx, cy, cz, spin_x, spin_r, visited, stack, cluster, beta);
            check_neighbor_ghost(lattice, cx, (cx+1+Lx)%Ly, cz, spin_x, spin_r, visited, stack, cluster, beta);
            check_neighbor_ghost(lattice, cx, (cx-1+Lx)%Ly, cz, spin_x, spin_r, visited, stack, cluster, beta);
            check_neighbor_ghost(lattice, cx, cy, (cz+1+Lx)%Lz, spin_x, spin_r, visited, stack, cluster, beta);
            check_neighbor_ghost(lattice, cx, cy, (cz-1+Lx)%Lz, spin_x, spin_r, visited, stack, cluster, beta);
