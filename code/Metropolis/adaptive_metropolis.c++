...
a Gaussian move. To generate a trial spin we add a Gaussian 
distributed random vector multiplied by a factor sigma to the initial 
spin vector and then normalize the result to obtain the new spin. 
The adaptive element of the algorithm is the factor sigma that 
multiplies the Gaussian distributed vector, which takes over the role 
of the opening angle in the small step move. This factor is changed 
after each step, depending on the acceptance rate, to reach an optimal 
acceptance rate of 50%. A more detailed description of the algorithm 
...
bool adaptive_metropolis(   Lattice3D<Spin> &lattice,
                            ...
                            flt const& maxTimeSeconds,/*sim time*/
                            u64 const& maxSteps,/*number of steps*/
                            Spin const& h, /*magnetic fiel vector*/
                            Spin const& k, /*anisotropy vector*/
                            flt const& maxFactor){
    // Initialize sigma with the max value, start timer, calculate 
    // Boltzmann factor and initialize counter of accepted steps;
    measure::Timer watch;
    flt beta = Beta(T);
    // Main Metropolis loop until number of steps or max time is
    // reached
    ...
    for(u64 step = 0; step < maxSteps; ++step){
        ...
        if (deltaE <= 0 || rng::rand_uniform() < exp(-deltaE * beta)){
            ...
            // The acceptance rate is given by the ratio of the 
            // accepted steps and the proposed steps.
